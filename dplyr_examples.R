...
filter(flights, month == 1, day == 1) #look at a subset of observations based on their values
jan1 <- filter(flights, month == 1, day == 1) #if you want to save the result from above you need the "<-" modifier 
(dec25 <- filter(flights, month == 12, day == 25)) #example of printing out results & saving them as a variable
filter(flights, month = 1) #example of needing "==" for equality instead of "="
filter(flights, month == 1) #example runs correctly
near(sqrt(2) ^ 2,  2) #use "near()" for approximations though instead of "=="
near(1 / 49 * 49, 1) #additional example
filter(flights, month == 11 | month == 12) #find all flights that occurred in november & december
nov_dec <- filter(flights, month %in% c(11, 12)) #this will select every row where x is one of the values in y
